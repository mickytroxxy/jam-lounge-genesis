...
const DeckB: React.FC<DeckBProps> = ({
  ...
  useEffect(() => {
    if((deckB?.position > 43) && (deckB?.position < 46) ){
      clearCurrentBid(deckB?.currentTrack)
    }
  },[deckB?.position])
  ...
  return (
    <DiscoLights isPlaying={deckB.isPlaying} deckId="B" enabled={discoLightsEnabled}>
      <div 
        className="glass-card p-4 animate-fade-in-up font-montserrat-light transition-colors hover:border-blue-500/50 border border-transparent" 
      ...
      <div className="mb-3">
        <WaveformDisplay
          audioUrl={deckB.currentTrack?.audioUrl || deckB.currentTrack?.url}
          ...
          loopOut={loopOut}
          onSeek={(pos) => {
            const audio = document.querySelector('#deckB-audio') as HTMLAudioElement;
      ...
      {/* Track Info */}
      <div className="bg-gradient-to-r from-blue-500/20 to-transparent rounded-lg px-3 py-2 mb-3">
        <p className="text-gray-300 text-xs truncate">
          {deckB.currentTrack ? `${deckB.currentTrack.artist} - ${deckB.currentTrack.title}` : 'Drop a track or select from library'}
        </p>
      </div>
      ...
      <div className="mb-3">
        <HotCuePads
          hotCues={hotCues}
          currentPosition={deckB.position || 0}
          isDisabled={!deckB.currentTrack}
          onSet={(index, pos) => {
